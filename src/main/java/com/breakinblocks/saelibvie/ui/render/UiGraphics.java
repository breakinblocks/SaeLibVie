...
public final class UiGraphics {
    ...
    public UiGraphics(GuiGraphicsExtractor gui, Theme theme, int mouseX, int mouseY, float partialTick) {
        this.gui = gui;
    ...
    public void pushScissor(Rect local) {
        gui.enableScissor(Math.min(local.x(), local.right()), Math.min(local.y(), local.bottom()),
                Math.max(local.x(), local.right()), Math.max(local.y(), local.bottom()));
        scissorDepth++;
    }
    ...
    public void blit(Identifier texture, int x, int y, int u, int v, int w, int h) {
        blit(texture, x, y, u, v, w, h, 256, 256);
    }

    public void blit(Identifier texture, int x, int y, float u, float v, int w, int h, int texW, int texH) {
        if (alpha <= 0f) return;
        gui.blit(RenderPipelines.GUI_TEXTURED, texture, x, y, u, v, w, h, texW, texH, tint());
    }

    public void blitScaled(Identifier texture, int x, int y, int w, int h, float u, float v, int uW, int vH, int texW, int texH) {
        if (alpha <= 0f) return;
        gui.blit(RenderPipelines.GUI_TEXTURED, texture, x, y, u, v, w, h, uW, vH, texW, texH, tint());
    }

    public void sprite(Identifier sprite, int x, int y, int w, int h) {
        if (alpha <= 0f) return;
        gui.blitSprite(RenderPipelines.GUI_TEXTURED, sprite, x, y, w, h, tint());
    }

    public void sprite(Identifier sprite, Rect r) {
        sprite(sprite, r.x(), r.y(), r.w(), r.h());
    }

    public void spriteRegion(Identifier sprite, int spriteW, int spriteH, int uOff, int vOff, int x, int y, int w, int h) {
        if (alpha <= 0f) return;
        gui.blitSprite(RenderPipelines.GUI_TEXTURED, sprite, spriteW, spriteH, uOff, vOff, x, y, w, h, tint());
    }

    public void atlasSprite(TextureAtlasSprite sprite, int x, int y, int w, int h, int tint) {
        if (alpha <= 0f) return;
        gui.blitSprite(RenderPipelines.GUI_TEXTURED, sprite, x, y, w, h, apply(tint));
    }
    ...
    public ScreenRectangle scissorArea() {
        return gui.peekScissorStack();
    }

    public void submitPictureInPicture(PictureInPictureRenderState state) {
        gui.submitPictureInPictureRenderState(state);
    }

    public static EntityRenderState entityRenderState(LivingEntity entity) {
        EntityRenderer<? super LivingEntity, ?> renderer = Minecraft.getInstance().getEntityRenderDispatcher().getRenderer(entity);
        EntityRenderState state = renderer.createRenderState(entity, 1f);
        state.shadowPieces.clear();
        state.outlineColor = 0;
        return state;
    }

    public void entity(EntityRenderState state, Rect local, float size, Vector3f translation,
                       Quaternionf rotation, @Nullable Quaternionf cameraAngle) {
        if (alpha <= 0f) return;
        gui.entity(state, size * scale, translation, rotation, cameraAngle,
                toAbsX(local.x()), toAbsY(local.y()), toAbsX(local.right()), toAbsY(local.bottom()));
    }
    ...
    public void itemWithDecorations(ItemStack stack, int x, int y, @Nullable String countText) {
        if (stack.isEmpty()) return;
        gui.item(stack, x, y);
        gui.itemDecorations(font, stack, x, y, countText);
    }
    ...
    public void text(Component text, int x, int y, int color, boolean shadow) {
        ...
        gui.text(font, text, x, y, apply(color), shadow);
    }
    ...
    public void flushTooltip() {
        ...
        if (request.stack != null) {
            gui.setTooltipForNextFrame(font, request.stack, request.x, request.y);
        } else if (request.raw != null) {
            gui.setComponentTooltipFromElementsForNextFrame(font, request.raw, request.x, request.y, ItemStack.EMPTY);
        } else if (request.lines != null) {
            gui.setTooltipForNextFrame(font, request.lines, request.extra, request.x, request.y);
        }
    }
