...
public final class ThemeLoader extends SimpleJsonResourceReloadListener {
    ...
    protected void apply(Map<ResourceLocation, JsonElement> map, ResourceManager resourceManager, ProfilerFiller profiler) {
        Map<ResourceLocation, JsonObject> themes = new LinkedHashMap<>();
    ...
    public static void loadConfig() {
        ...
        try (Reader reader = Files.newBufferedReader(file)) {
            ...
            Map<ResourceLocation, JsonObject> overrides = new LinkedHashMap<>();
            if (obj.has("overrides") && obj.get("overrides").isJsonObject()) {
                for (Map.Entry<String, JsonElement> entry : obj.getAsJsonObject("overrides").entrySet()) {
                    if (entry.getValue().isJsonObject()) {
                        overrides.put(ResourceLocation.parse(entry.getKey()), entry.getValue().getAsJsonObject());
                    }
                }
            }
            if (obj.has("opacity")) {
            ...
            if (obj.has("default")) {
                Themes.setDefault(ResourceLocation.parse(obj.get("default").getAsString()));
            }
        } catch (Exception e) {
