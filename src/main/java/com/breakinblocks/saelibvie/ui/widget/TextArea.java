...
public class TextArea extends Widget {
    ...
    protected boolean onMouseClicked(double lx, double ly, int button) {
        ...
        requestFocus();
        model.setSelecting(Modifiers.shift());
        seekMouse(lx, ly);
        dragging = true;
        return true;
    }
    ...
    protected boolean onKeyPressed(int key, int scanCode, int modifiers) {
        ...
        if ((key == InputConstants.KEY_RETURN || key == InputConstants.KEY_NUMPADENTER) && Modifiers.shift(modifiers)) {
            return false;
        }
        KeyEvent event = new KeyEvent(key, scanCode, modifiers);
        if (!editable) {
            if (Modifiers.isCopy(key, modifiers)) {
                Minecraft.getInstance().keyboardHandler.setClipboard(model.getSelectedText());
                return true;
            }
            if (Modifiers.isSelectAll(key, modifiers)) {
                selectAll();
                return true;
            }
            return switch (key) {
                case InputConstants.KEY_LEFT, InputConstants.KEY_RIGHT, InputConstants.KEY_UP, InputConstants.KEY_DOWN,
                     InputConstants.KEY_HOME, InputConstants.KEY_END -> model.keyPressed(event);
                default -> false;
            };
        }
        boolean handled = model.keyPressed(event);
        if (handled) {
            revealCursor();
        }
        return handled;
