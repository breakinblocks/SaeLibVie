...
public class Viewport3D extends Widget {
    ...
    public interface Renderer {
        void render(PoseStack pose, MultiBufferSource.BufferSource buffers, SubmitNodeCollector collector, float partialTick);
    }
    ...
    public static Viewport3D ofBlock(Supplier<BlockState> state) {
        return new Viewport3D((pose, buffers, collector, partial) -> {
    ...
    protected void paint(UiGraphics g) {
        ...
        g.submitPictureInPicture(new ViewportRenderState(
                renderer,
                yaw,
                pitch,
                centerX,
                centerY,
                centerZ,
                lightMode == LightMode.ENTITY,
                g.partialTick(),
                g.toAbsX(inner.x()),
                g.toAbsY(inner.y()),
                g.toAbsX(inner.right()),
                g.toAbsY(inner.bottom()),
                scale,
                g.scissorArea()));
    }
