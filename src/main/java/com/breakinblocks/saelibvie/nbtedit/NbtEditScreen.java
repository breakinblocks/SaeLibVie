...
public class NbtEditScreen extends FormScreen {
    ...
    public static Identifier spriteFor(int type, boolean open) {
    ...
    private void editLeaf(Node node) {
        ...
        if (tag instanceof StringTag) {
            initial = tag.asString().orElse("");
            parser = text -> text;
        } else if (tag instanceof LongTag) {
            initial = Long.toString(((NumericTag) tag).longValue());
    ...
    final class Node {
        ...
        void build() {
            ...
            if (tag instanceof CompoundTag compound) {
                ...
                if (compound.get("id") instanceof StringTag && compound.get("count") instanceof NumericTag) {
                    try {
                        Optional<ItemStack> parsed = ItemStack.CODEC
                                .parse(ClientUtil.registryAccess().createSerializationContext(NbtOps.INSTANCE), compound)
                                .result();
        ...
        List<Integer> acceptedTypes() {
            ...
            if (tag instanceof ListTag list) {
                ...
                return List.of((int) list.get(0).getId());
            }
        ...
        void setChildTag(String childKey, Tag value) {
            if (tag instanceof CompoundTag compound) {
                ...
                int index = Integer.parseInt(childKey);
                if (index >= 0 && index < array.length && value instanceof NumericTag n) array[index] = n.byteValue();
                tag = new ByteArrayTag(array);
            } else if (tag instanceof IntArrayTag ints) {
                int[] array = ints.getAsIntArray().clone();
                int index = Integer.parseInt(childKey);
                if (index >= 0 && index < array.length && value instanceof NumericTag n) array[index] = n.intValue();
        ...
        void append(Tag value) {
            if (tag instanceof ListTag list) {
                ...
                next[array.length] = value instanceof NumericTag n ? n.byteValue() : 0;
                tag = new ByteArrayTag(next);
            } else if (tag instanceof IntArrayTag ints) {
                int[] array = ints.getAsIntArray();
                int[] next = Arrays.copyOf(array, array.length + 1);
                next[array.length] = value instanceof NumericTag n ? n.intValue() : 0;
                tag = new IntArrayTag(next);
            }
            propagate();
            build();
        ...
        String valueText() {
            if (tag instanceof ByteTag || tag instanceof ShortTag || tag instanceof IntTag) return Integer.toString(((NumericTag) tag).intValue());
            if (tag instanceof LongTag) return Long.toString(((NumericTag) tag).longValue());
            if (tag instanceof FloatTag || tag instanceof DoubleTag) return Double.toString(((NumericTag) tag).doubleValue());
