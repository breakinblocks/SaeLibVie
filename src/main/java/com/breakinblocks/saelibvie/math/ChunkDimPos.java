...
public record ChunkDimPos(ResourceKey<Level> dimension, ChunkPos chunkPos) implements Comparable<ChunkDimPos> {
    ...
    public int compareTo(ChunkDimPos other) {
        int byDim = dimension.identifier().toString().compareTo(other.dimension.identifier().toString());
        if (byDim != 0) return byDim;
        return Long.compare(x() & 0xFFFFFFFFL | (z() & 0xFFFFFFFFL) << 32, other.x() & 0xFFFFFFFFL | (other.z() & 0xFFFFFFFFL) << 32);
    }
}
