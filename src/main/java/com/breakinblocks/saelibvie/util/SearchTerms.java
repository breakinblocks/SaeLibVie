...
public final class SearchTerms {
    ...
    public record Term(Kind kind, String value, @Nullable Identifier tag) {
        boolean matches(Identifier id, String displayName, Predicate<Identifier> tagMatcher) {
    ...
    public static SearchTerms parse(String text) {
        ...
        for (String raw : text.toLowerCase(Locale.ROOT).split(" +")) {
            ...
            if (raw.charAt(0) == '@') {
                ...
                terms.add(new Term(Kind.TAG, value, value.isEmpty() ? null : Identifier.tryParse(value)));
            } else {
