...
import com.breakinblocks.saelibvie.ui.render.ViewportRenderState;
import com.breakinblocks.saelibvie.ui.render.ViewportRenderer;
...
public final class SaeLibVieClient {
    public static final KeyMapping.Category CATEGORY = new KeyMapping.Category(SaeLibVie.id("main"));
    ...
    public static void init(IEventBus modBus, ModContainer container) {
        ...
        modBus.addListener(SaeLibVieClient::registerPictureInPictureRenderers);
    ...
    private static void registerReloadListeners(AddClientReloadListenersEvent event) {
        event.addListener(SaeLibVie.id("themes"), new ThemeLoader());
    }

    private static void registerPictureInPictureRenderers(RegisterPictureInPictureRenderersEvent event) {
        event.register(ViewportRenderState.class, ViewportRenderer::new);
    }
    ...
    private static void registerClientCommands(RegisterClientCommandsEvent event) {
        event.getDispatcher().register(Commands.literal(SaeLibVie.MOD_ID)
                .then(Commands.literal("hud").executes(ctx -> {
                    Minecraft.getInstance().execute(HudEditScreen::open);
                    return 1;
                }))
                .then(Commands.literal("demo").executes(ctx -> {
                    Minecraft.getInstance().execute(() -> Minecraft.getInstance().setScreen(new DemoScreen(null)));
                    return 1;
                }))
                .then(Commands.literal("themes").executes(ctx -> {
                    StringBuilder builder = new StringBuilder();
                    for (Identifier id : Themes.ids()) {
                ...
                .then(Commands.literal("theme")
                        .then(Commands.argument("id", StringArgumentType.string())
                                ...
                                .executes(ctx -> {
                                    Identifier id = Identifier.tryParse(StringArgumentType.getString(ctx, "id"));
