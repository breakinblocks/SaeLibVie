...
SaeLibVie is a shared library mod for NeoForge 26.1.2 (Java 25). It exists so that Breakin Blocks mods can stop re-implementing the same screens, overlays and helpers. It ships:
...
Version scheme is `<minecraft>-<mod>`, currently `26.1.2-1.0.0`. The jar is `saelibvie-26.1.2-1.0.0.jar`.
...
dependencies {
    implementation "com.breakinblocks.saelibvie:saelibvie:26.1.2-1.0.0"
}
```
...
Access transformers are in `META-INF/accesstransformer.cfg`, mixins in `saelibvie.mixins.json`. Both go through ModDevGradle and are declared in `src/main/resources/META-INF/neoforge.mods.toml`.
...
| `ui.render` | client | `UiGraphics`, `Painter`, `ViewportRenderState`, `ViewportRenderer` |
...
| `UiGraphics` | Wrapper over `GuiGraphicsExtractor` with a translate/scale stack, alpha stack, theme stack, local-to-screen scissor conversion, fills, gradients, outlines, bevels, sprites, atlas sprites (fluids), items, entity and picture-in-picture submission, text helpers (fit with ellipsis, wrap, header text) and tooltip requests. |
...
3D: `EntityView` (mouse follow, auto spin, or orbit drag) and `Viewport3D` (orbit and zoom; `Viewport3D.ofBlock` renders a block state). Both go through the 26.1 picture-in-picture pipeline: `EntityView` builds an `EntityRenderState`, `Viewport3D` submits a `ViewportRenderState` handled by `ViewportRenderer`, which the mod registers on `RegisterPictureInPictureRenderersEvent`.
...
- `IdentifierMixin`: when `StringUtil.ignoreIdentifierErrors` is set, the namespace and path validation predicates accept any string.
...
src/main/resources/META-INF/neoforge.mods.toml  mod metadata (expanded at build time)
src/test/java/                               JUnit tests
```

## License

SaeLibVie is made available under two sets of terms, the PolyForm Shield License 1.0.0 and the PolyForm
Perimeter License 1.0.0, both reproduced in full in `LICENSE.md`. Your use of the software must comply
with both.
